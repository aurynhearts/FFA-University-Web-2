(() => {
  ...
  const CHAR_COUNT_OPTIONS = ["1 Character", "2 Characters", "3 Characters"];

  const LOCK_TYPE = [RANDOM_SENTINEL, ...TYPES];
  const LOCK_AGE = [RANDOM_SENTINEL, ...Object.keys(AGE_TAGS)];
  const LOCK_POSITION = [RANDOM_SENTINEL, ...POSITIONS];
  const LOCK_DEPARTMENT = [RANDOM_SENTINEL, ...DEPARTMENTS];
  const LOCK_HAIR_COLOR = [RANDOM_SENTINEL, ...HAIR_COLORS];
  const LOCK_HAIR_STYLE = [RANDOM_SENTINEL, ...HAIR_STYLES];
  const LOCK_EYE_COLOR = [RANDOM_SENTINEL, ...EYE_COLORS];
  const LOCK_BODY_TYPE = [RANDOM_SENTINEL, ...BODY_TYPES];
  const LOCK_BACKGROUND = [RANDOM_SENTINEL, ...Object.keys(BACKGROUNDS)];

  const LOCK_DEFS = [
    ["Type", LOCK_TYPE, "type"],
    ["Age", LOCK_AGE, "age"],
    ["Position", LOCK_POSITION, "position"],
    ["Department", LOCK_DEPARTMENT, "department"],
    ["Hair Color", LOCK_HAIR_COLOR, "hair_color"],
    ["Hair Style", LOCK_HAIR_STYLE, "hair_style"],
    ["Eye Color", LOCK_EYE_COLOR, "eye_color"],
    ["Body Type", LOCK_BODY_TYPE, "body_type"],
  ];

  function createEmptyLocks() {
    return {
      type: RANDOM_SENTINEL,
      age: RANDOM_SENTINEL,
      position: RANDOM_SENTINEL,
      department: RANDOM_SENTINEL,
      hair_color: RANDOM_SENTINEL,
      hair_style: RANDOM_SENTINEL,
      eye_color: RANDOM_SENTINEL,
      body_type: RANDOM_SENTINEL,
    };
  }

  function createCharacterNsfwSettings() {
    return {
      outfit_mode: "Normal",
      act_mode: "Random",
      act_options: [],
  ...
  function createBaseNsfwSettings() {
    return {
      act_mode: "Normal",
      act_options: [],
      act_options_by_mode: {},
  ...
  const appState = {
    char_count: 1,
    background: RANDOM_SENTINEL,
    character_locks: [createEmptyLocks()],
    nsfw_settings: [createCharacterNsfwSettings()],
    base_nsfw_settings: createBaseNsfwSettings(),
    last_result: null,
  };

  function getCharCountFromLabel(label) {
    const match = String(label).match(/^(\d+)/);
    return match ? Number(match[1]) : 1;
  }

  function syncCharacterCount(charCount) {
    appState.char_count = charCount;

    while (appState.character_locks.length < charCount) {
      appState.character_locks.push(createEmptyLocks());
    }
    appState.character_locks = appState.character_locks.slice(0, charCount);

    while (appState.nsfw_settings.length < charCount) {
      appState.nsfw_settings.push(createCharacterNsfwSettings());
    }
    appState.nsfw_settings = appState.nsfw_settings.slice(0, charCount);
  }

  function setCharacterLocks(index, locks) {
  ...
  function clearCharacterLocks(index) {
    appState.character_locks[index] = createEmptyLocks();
  }
  ...
  window.FUGState = {
    ...
    appState,
    createEmptyLocks,
    createCharacterNsfwSettings,
    createBaseNsfwSettings,
